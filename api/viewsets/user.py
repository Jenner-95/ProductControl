...
class UserViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
            return User.objects.filter(is_active=True, username=self.request.user)
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            # # Extrae el arreglo de permisos
            # permissions = request.data.pop('permissions')
            # role = request.data.pop('role')
            
            # instanceRole = None
            # # Creación de permisos
            # if not Role.objects.filter(pk=role).exists(): 
            #     instanceRole = Role.objects.create(name='')
            #     instanceRole.permissions.set(permissions)
    ...
    def send_email_password(self, user, password ):
        ...
        subject = f'Bienvenido ${user.first_name} a ONLINEDEV.GT'
        from_email = 'ONLINEDEV.GT  <noreply@school.com>'
        content = render_to_string(
